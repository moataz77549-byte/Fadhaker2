...
async function handleQuranProxy(
  ...
  try {
    // Quranpedia's subject index is interpretive metadata, never Quran text.
    // Keep the upstream address fixed and validate its shape before serving it.
    if (sub === 'topics') {
      const cached = cacheGet<unknown>('quranpedia:topics:v1', 24 * 3600_000);
      if (cached) return jsonResponse(cached, 200, requestId);
      const response = await fetch('https://api.quranpedia.net/v1/topics', {
        headers: { Accept: 'application/json' },
        signal: AbortSignal.timeout(12000),
      });
      if (!response.ok) return errorResponse('TOPICS_UPSTREAM', 'Topic catalog unavailable', 502, requestId);
      const body: unknown = await response.json();
      if (!Array.isArray(body) || body.length > 30000 || !body.every((item) =>
        item && typeof item === 'object' && Number.isInteger(item.id) &&
        typeof item.name === 'string' && typeof item.ayahs === 'string')) {
        return errorResponse('TOPICS_INVALID', 'Invalid topic catalog', 502, requestId);
      }
      const payload = {
        ...
        licenseUrl: 'https://api.quranpedia.net/dumps/LICENSE.md',
        topics: body,
      };
      cacheSet('quranpedia:topics:v1', payload);
